...
Overflow is truncated with `…` by default. To softwrap long rows instead, enable wrap:
...
When `wrap = true`, a configured row that would overflow wraps onto continued lines indented with `   ` instead of truncating. The entry grows taller and scroll calculations include the extra lines. Wrapping applies only to the expanded desktop sidebar; collapsed and mobile views keep their compact layouts.
